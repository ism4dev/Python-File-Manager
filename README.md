...
- Execução/Abertura segura de arquivos (`.exe`, `.bat`, `.py` e outros arquivos perigosos são bloqueados)
...
> Caso queira, o programa pode ser executado diretamente via binário. Os executáveis já estão incluídos na pasta `builds`:

```
builds/
├── windows/    # Python File Manager.exe
└── linux/      # Python File Manager.elf
```

No Windows, rode: `builds/windows/Python File Manager.exe`
No Linux, rode: `builds/linux/Python File Manager.elf`
...
```bash
git clone https://github.com/ism4dev/Python-File-Manager
...
├── Python File Manager.py         # Script principal
└── builds/                        # Executáveis já compilados
    ├── windows/                   # Python File Manager.exe
    └── linux/                     # Python File Manager.elf
```
